    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 flex max-h-[80vh] w-full max-w-lg flex-col rounded-xl bg-white shadow-2xl">
        <div className="border-b border-neutral-200 px-6 py-4">
          <h3 className="text-lg font-semibold text-neutral-900">Выберите блюдо</h3>
          <input
            type="text"
            autoFocus
            value={search}
            ...
            className="mt-3 block w-full rounded-lg border border-neutral-200 px-3 py-2 text-sm shadow-sm focus:border-royal-500 focus:ring-1 focus:ring-royal-500/20 focus:outline-none"
          />
        </div>

        <div className="flex-1 overflow-y-auto px-6 py-4">
          {filtered.length === 0 ? (
            <p className="py-8 text-center text-sm text-neutral-400">Ничего не найдено</p>
          ) : (
        ...
        <div className="border-t border-neutral-200 px-6 py-3">
          <button
            onClick={onClose}
            className="w-full rounded-lg border border-neutral-200 px-4 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-50"
          >
            Закрыть
          </button>
        </div>
