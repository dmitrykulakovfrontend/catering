    ...
    <div className="sticky bottom-0 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-neutral-200 bg-white px-6 py-4 shadow-lg">
      <div className="flex flex-wrap items-baseline gap-x-5 gap-y-1 text-sm">
        ...
        {persons > 0 && (
          <div className="text-xs text-neutral-500">
            ...
            <span className="font-semibold text-neutral-700">
              {Math.round(total / persons).toLocaleString("ru-RU")} ₽
            </span>{" "}
            / чел.
          </div>
        )}
        <div className="text-xs text-neutral-500">
          Меню:{" "}
          <span className="font-semibold text-neutral-700">
            {menuSubtotal.toLocaleString("ru-RU")} ₽
          </span>
          {servicesCount > 0 && (
            <>
              {" · "}Услуги:{" "}
        ...
        {Object.keys(totalWeight).length > 0 && (
          <div className="text-xs text-neutral-500">
            Выход:{" "}
            <span className="font-semibold text-neutral-700">
              {formatWeightMap(totalWeight)}
            </span>
            {persons > 0 && (
              <>
                {" · ≈ "}
                <span className="font-semibold text-neutral-700">
                  {formatWeightMap(perPerson(totalWeight, persons))}
                </span>{" "}
                / чел.
              </>
            )}
      ...
      <div className="flex items-center gap-3">
        ...
        {isDirty && (
          ...
          >
            ...
            >
              <path
                d="M2 6.5a4.5 4.5 0 1 0 1.3-3.2M2 2v3h3"
                stroke="currentColor"
                strokeWidth="1.3"
                strokeLinecap="round"
        ...
        {previewSlug && (
          ...
          >
            ...
            >
              <path
                d="M8 2h4v4M11.5 2.5L6 8M6 3H3a1 1 0 0 0-1 1v7a1 1 0 0 0 1 1h7a1 1 0 0 0 1-1V8"
                stroke="currentColor"
                strokeWidth="1.3"
                strokeLinecap="round"
                strokeLinejoin="round"
