...
export default function SectionCard({
  ...
  const titleClass = titleError
    ? `flex-1 min-w-32 rounded-md border border-red-400 bg-red-50/40 px-2 py-1 text-sm font-medium ${INPUT_ERROR_FOCUS_RING}`
    : `flex-1 min-w-32 rounded-md border border-neutral-200 bg-white px-2 py-1 text-sm font-medium ${INPUT_FOCUS_RING}`;
  return (
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-neutral-100 bg-neutral-50 px-5 py-3">
        ...
        <div className="flex gap-1">
          <button
            onClick={() => onMove(-1)}
            disabled={isFirst}
            className="rounded px-1 text-neutral-400 hover:text-neutral-600 disabled:opacity-30"
          >
            ↑
          </button>
          <button
          ...
          <div className="divide-y divide-neutral-50">
            {section.items.map((item, iIdx) => (
              <SectionItemRow
                ...
                errors={{
                  name: errors[`sections.${index}.items.${iIdx}.name`],
                  quantity: errors[`sections.${index}.items.${iIdx}.quantity`],
                  pricePerUnit:
                    errors[`sections.${index}.items.${iIdx}.pricePerUnit`],
                  weight: errors[`sections.${index}.items.${iIdx}.weight`],
                }}
